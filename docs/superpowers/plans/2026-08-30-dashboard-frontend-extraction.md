...
**The reconstruction test retires at the start of Phase 1.** `ReconstructionTests` in `tests/test_dashboard_frontend.py`, together with `tests/fixtures/dashboard_baseline.html`, exist to prove one thing: that this extraction moved bytes without changing them. That proof is only meaningful against a frozen pre-refactor snapshot. The first legitimate CSS or JS edit in Phase 1 will fail it, and the only way to make it pass again is regenerating the baseline fixture from the edited files — at which point the test compares the files to themselves and proves nothing, while the fixture goes on carrying around 100KB of duplicated bytes forever. Delete `ReconstructionTests` and `tests/fixtures/dashboard_baseline.html` at the start of Phase 1. Keep `RouteTests`: it exercises real behavior (the index route, static asset serving, cache headers, 404s) rather than a frozen snapshot, and stays useful for as long as the dashboard has routes.
